...
export default function Input({ id, type = '', min = 0, max, step = 1, value, setValue, notEditable = false }) {
    ...
    const addSymbol = (event) => {
        ...
        let tempValue = event.target.value;
        if (!(String(textValue).charAt(0) == '\u20B9') && type == 'rupees') {
            tempValue = ((type === 'rupees') ? '\u20B9' : '') + Number(tempValue).toLocaleString("en-In");
        }
        if (!(String(textValue).charAt(textValue.length - 1) == '%')) {
        ...
        <div className={styles.inputBox}>
            <div className={' flex justify-between flex-warp items-center  '}> {/*[@media(max-width:500px)]:h-[54px]*/}
                ...
                <div className=' w-[39%] lg:ml-[25px] ml-[10px] min-w-[150px] [@media(max-width:500px)]:min-w-[120px]  text-[#1B1C20] '>
                    {notEditable ?
                        <div className={'h-[40px] w-full text-[#1B1C20] bg-[#DCE3EE] border-2 border-solid border-[#C7CFDD] rounded-[100px] text-center text-[14px] font-semibold [@media(min-width:1920px)]:text-[18px] flex items-center justify-center'}>
                            {textValue}
                        </div> :
                        <input
                            type="text"
            ...
            {(value < min) ? <div className=' text-right text-[#FF7D7D] text-[14px] [@media(max-width:500px)]:text-[10px] font-normal -mt-[0px] -mb-[20px] [@media(max-width:500px)]:-mt-[3px] [@media(max-width:500px)]:-mb-[17px]'>Minimum value is {min.toLocaleString('en-In')}</div> : (raiseMaxError) ? <div className=' text-right text-[#FF7D7D] text-[14px] [@media(max-width:500px)]:text-[10px] font-normal -mt-[0px] -mb-[20px] [@media(max-width:500px)]:-mt-[3px] [@media(max-width:500px)]:-mb-[17px]'>Maximum value is {max.toLocaleString('en-In')}</div> : ''}
        </div>
    )
}