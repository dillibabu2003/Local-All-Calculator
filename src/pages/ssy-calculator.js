...
export default function Home() {
  ...
  const [totalInterest, setTotalInterest] = useState(289538);
  const [years, setYears] = useState(2037);
  const [investment, setInvestment] = useState(10000);

  const [isLineChart, setCheck] = useState(true);

  const [corpusPoints, setCorpusPoints] = useState([
  ...
  const [investmentPoints, setInvestmentPoints] = useState([
    10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, 100000, 110000, 120000, 130000, 140000, 150000, 150000, 150000, 150000, 150000, 150000, 150000
  ]);
  ...
  function calculate() {
    if ((yearlyInvestment >= minYearlyInvestment && yearlyInvestment <= maxYearlyInvestment) && (age >= minAge && age <= maxAge)) {
      ...
      let points1 = [], points2 = [], temage = age;
      let conyear = 0, i = 1;
      ...
      for (i = 1; i <= 11 + (10 - age); i++) {
        ...
        points1.push(Math.round(cumulativeBalance));
        points2.push(conyear);
      }
      temage--;
      ...
      setMaturityValue(points1[points1.length - 1]);

      // let plot = [];
      // for (let i = 0; i <= 15 - age; i++) {
      //   plot.push(points[i]);
      // }
      setCorpusPoints(points1);
      setInvestmentPoints(points2);
    }
          ...
          <CalculatorWrapper>
            ...
            <Calculator calculate={calculate}>
              ...
              <InputBoxWrapper>
                ...
                <div className="flex flex-row justify-between ">
                  <h3 className=" mt-6 w-[58%] ">Current Interest Rate</h3>
                  <div className={'mt-4 w-[39%] lg:ml-[25px] ml-[10px] min-w-[150px] [@media(max-width:500px)]:min-w-[120px]   h-[40px]  text-[#1B1C20] bg-[#DCE3EE] border-2 border-solid border-[#C7CFDD] rounded-[100px] text-center text-[14px] font-semibold [@media(min-width:1920px)]:text-[18px] flex items-center justify-center'}>
                    7.6%
                  </div>
                </div>
            ...
            <div className={"lg:w-[50%] "}>
              ...
              <div className={" relative object-right-top "}>
                {isLineChart ? (
                  <>
                    <LineChart key='' points={optionsForLine} />
          ...
          contents={[
            ["What is Sukanya Samriddhi Yojana?",
              <>
                ...
                <br />
                <div>Under SSY, parents or guardians can open a savings account in the name of a girl child aged 10 years or below and make regular deposits until the girl reaches the age of 15. The deposit amount is eligible for tax benefits under Section 80C of the Income Tax Act, and the interest earned on the deposit is tax-free.</div>
                <br />
            ...
            ["When does my SSY investment mature? ",
              <>
                <div>The maturity period of the Sukanya Samriddhi Yojana (SSY) savings scheme in India is 21 years from the date of opening the account.</div>
                <br />
            ...
            ["Is withdrawal permitted before maturity?", 'Withdrawals under Sukanya Samriddhi Account, are allowed only after the girl child turns 18 years and can be used for marriage or higher education.'],
          ]}
        />
