...
import RDDoughnutChart from "../Components/RDDoughnutChart.js";
...
export default function Home() {
    ...
    const [maturityGraphPoints, setMaturityGraphPoints] = useState([1246241, 2582035, 4013819, 5548489, 7193439, 8956593, 10846447, 12872103, 15043320, 17370560]);
    const [investmentGraphPoints, setInvestmentGraphPoints] = useState([1200000, 2400000, 3600000, 4800000, 6000000, 7200000, 8400000, 9600000, 10800000, 12000000]);

    // For two graph lines...
    const optionsAndDualLine = [
        {
            data: maturityGraphPoints,
            color: '#00DD6F',
            fillColor: {
    ...
    function calculateGraphPoints() {
        ...
        for (let i = 1; i <= timePeriod * 12; i++) {
            ...
            if (i % 12 == 0) {
                investmentPoints.push(totalInvestment * i);
                maturityPoints.push(totalAmount);
            }
            totalAmount += totalInvestment;
        }
        setMaturityGraphPoints(maturityPoints);
        setInvestmentGraphPoints(investmentPoints);
    }
                        ...
                        <div className={"lg:w-[50%]"}>
                            ...
                            <div className={" relative object-right-top [@media(min-width:200px)]:h-auto md:w-[100%]"}>
                                ...
                                <div className={" relative object-right-top"}>
                                    {isLineChart ? (
                                        <>
                                            <LineChart points={optionsAndDualLine} />

                                        </>
                                    ) : (
                                        <>
                                            <RDDoughnutChart
                                                totalInterest={maturityValue - (totalInvestment * timePeriod * 12)}
                                                investmentAmount={totalInvestment * timePeriod * 12}
