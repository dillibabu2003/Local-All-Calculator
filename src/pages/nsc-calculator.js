...
import LineChart from "../Components/Cagr/LineChartNsc.js";
...
export default function Home() {
  ...
  const [corpusPoints, setCorpusPoints] = useState([
    2500000, 2675000, 2862250, 3062608, 3276990, 3506379
  ]);

  const [intialInvestementPoints, setIntialInvestementPoints] = useState([
  ...
  function calculateGraphPoints() {
    ...
    for (let i = 0; i <= 4; i++) {
      value += value * Math.pow((1 + 0.07 / tempCompoundingFrequency), tempCompoundingFrequency) - value;
      points1.push(value);
      points2.push(totalInvestment);
    }
    setCorpusPoints(points1);
    setIntialInvestementPoints(points2);
  }
